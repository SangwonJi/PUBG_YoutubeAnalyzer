"""Fix incorrectly classified Samsung collabs and reclassify."""

import sqlite3
from tqdm import tqdm

# 삼성 콜라보 키워드 (정확한 것만)
SAMSUNG_KEYWORDS = ['GALAXY', 'NOTE9', 'NOTE 9', 'SAMSUNG', 'NOTE10', 'NOTE 10']
...
def main():
    ...
    # 1. 잘못 분류된 Samsung 콜라보 되돌리기
    cursor.execute("""
        UPDATE videos 
        SET is_collab = 0, 
            collab_partner = NULL,
            collab_category = NULL
        WHERE collab_partner = 'Samsung'
    """)
    reset_count = cursor.rowcount
    print(f"Reset incorrectly classified Samsung: {reset_count}")
    
    # 2. 정확한 Samsung 콜라보 찾기
    ...
    print(f"Samsung collabs found: {samsung_count}")
    
    # 3. 콜라보 영상 content_type 설정
